...
def chart_monthly_registrations(df):
    ...
    ax.set_title('Article 10 — Monthly Registration Volume')
...
def chart_yearly_registrations(df):
    ...
    ax.set_title('Article 10 — Yearly Registration Volume')
    ax.set_ylabel('Dossiers Registered')
...
def chart_status_breakdown(df):
    ...
    # "No data" = neither resolved nor has a scheduled term
    ...
    ax.set_title('Article 10 — Dossier Status Breakdown')
    fig.tight_layout()
...
def chart_resolution_rate_by_year(df):
    ...
    ax.set_title('Article 10 — Resolution Rate by Registration Year')
...
def chart_wait_time_distribution(df):
    ...
    resolved = resolved[resolved > 0]  # Drop nonsensical values

    fig, ax = plt.subplots()
    ax.hist(resolved, bins=80, color=COLOR_PRIMARY, edgecolor='white', linewidth=0.3, zorder=3)
    ...
    ax.set_title('Article 10 — Processing Duration Distribution (Resolved Dossiers)')
...
def chart_median_wait_by_year(df):
    ...
    ax.set_title('Article 10 — Median Processing Time by Registration Year')
    ax.set_ylabel('Median Wait (days)')
    ax.set_xlabel('Registration Year')
    ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))

    # Add year labels in years
...
def print_summary(df):
    ...
    print('\n' + '=' * 60)
    print('  ARTICLE 10 — DESCRIPTIVE SUMMARY')
    print('=' * 60)
...
def main():
    ...
    print_summary(df)
    print('All Article 10 descriptive charts generated successfully.')
