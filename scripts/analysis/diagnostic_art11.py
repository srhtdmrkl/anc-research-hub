...
def chart_throughput_vs_backlog(df):
    ...
    ax1.set_title('Article 11 — Annual Throughput vs. Cumulative Backlog')
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(CHART_DIR, 'throughput_vs_backlog_art11.png'))
    plt.close(fig)
    print('  ✓ throughput_vs_backlog_art11.png')

    print('\n  Yearly Intake vs. Output:')
    for year, row in combined.iterrows():
        print(f'    {year}: Intake={int(row["Intake"]):>8,}  Resolved={int(row["Resolved"]):>8,}  '
              f'Net={int(row["Net"]):>+8,}  Backlog={int(row["Cumulative Backlog"]):>9,}')
...
def chart_seasonality(df):
    ...
    ax.set_title('Art. 11 — Registrations by Calendar Month')
    ax.set_ylabel('Total Dossiers')
    ax.set_xticks(range(1, 13))
    ax.set_xticklabels(months, fontsize=8)
    ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda v, _: f'{v:,.0f}'))

    # Resolutions — EXCLUDE imputed July 1 dates
    ...
    ax.set_title('Art. 11 — Resolutions by Calendar Month\n(excl. imputed July 1 dates)')
    ax.set_ylabel('Total Dossiers')
    ax.set_xticks(range(1, 13))
    ax.set_xticklabels(months, fontsize=8)
    ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda v, _: f'{v:,.0f}'))
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(CHART_DIR, 'seasonality_art11.png'))
    plt.close(fig)
    print('  ✓ seasonality_art11.png')
...
def main():
    ...
    print('\nAll Article 11 diagnostic charts generated successfully.')
