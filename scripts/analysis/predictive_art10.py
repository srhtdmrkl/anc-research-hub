...
def chart_intake_forecast(df):
    ...
    ax.set_title('Article 10 — Monthly Intake Demand Forecast (2021–2028)\nContinuing Post-Law 14 Decay')
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(CHART_DIR, 'intake_forecast_art10.png'))
    plt.close(fig)
    print('  ✓ intake_forecast_art10.png')
...
def chart_backlog_projection(df, projected_annual_intake):
    ...
    # Project forward: use Law 14 adjusted intake, but keep recent output baseline
    ...
    ax.plot(proj_years, proj_backlog, marker='s', color=COLOR_DANGER,
            linewidth=1.5, markersize=5, linestyle='--', zorder=3, label=f'Projected (Law 14 adjustment)')

    ax.axvspan(2025.5, 2030.5, alpha=0.06, color='gray')
    ax.set_title('Article 10 — Cumulative Backlog Projection (Law 14 Adjusted)')
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(CHART_DIR, 'backlog_projection_art10.png'))
    plt.close(fig)
    print('  ✓ backlog_projection_art10.png')
...
def print_queue_wait(current_backlog, annual_output):
    if annual_output > 0:
        ...
        print('  ⚠ Cannot estimate queue wait — annual output is zero.')


# ── Q7: Kaplan-Meier Survival Analysis ──────────────────────────────────────

def chart_survival_analysis(df):
    # Reference date: latest known date in data
    ref_date = df['DATA ÎNREGISTRĂRII'].max()

    # For resolved: duration = wait_days, event_observed = True
    # For pending: duration = ref_date - registration_date, event_observed = False (censored)
    ...
    ax.set_title('Article 10 — Kaplan-Meier Survival Curve (Time to Resolution)')
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(CHART_DIR, 'survival_curve_art10.png'))
    plt.close(fig)
    print('  ✓ survival_curve_art10.png')

    # Naive median (resolved only)
...
def main():
    ...
    print('\nAll Article 10 predictive charts generated successfully.')
